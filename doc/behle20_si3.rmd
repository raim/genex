...
mu <- 0.033 # h-1, growth rate, derived from OD curve
dp <- 0.064  # h-1, protein degradation rate, ASV-tagged mVenus
beta <- mu + dp # h-1
...
l <- 1   # uninduced "leaky" transcription, a.u. * h-1
v <- 100 # maximal induced transcription, a.u. * h-1
...
2. Calculate $\mu$ and $\beta =\mu + \delta_P$ from rhamnose step-down experiment to get
 $\delta_P$,
3. Estimate $y_0$ and leaky rate $l$ from $I(0)=0$ (no inducer) data for each promoter,
...
\begin{table}[ht]
  \begin{tabular}{l|rr|rrr|rrr}
    Promoter & $\mu$, h$^{-1}$ & $\delta_P$, h$^{-1}$ & $y_0$, a.u & $n$ & $l$, h$^{-1}$ &  $v$, h$^{-1}$ & $K$, $\mu$Mol & $\delta$, h$^{-1}$\\\hline
    P$_\text{vanPP}$ &0.033 &0.064 & 0  &4&0   & 696.2& 96.3&0.031\\
